...
El bootstrap requiere siempre `SEED_ADMIN_EMAIL` y `SEED_ADMIN_PASSWORD` de al
menos 12 caracteres; no existe administrador ni contraseña predeterminada.
...
4. Crea el primer admin una sola vez desde un shell seguro: `cd server && ALLOW_PRODUCTION_BOOTSTRAP=yes npm run seed`, con `SEED_ADMIN_EMAIL` y `SEED_ADMIN_PASSWORD` definidos. El arranque normal nunca ejecuta seed.
