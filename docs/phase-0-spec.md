...
- Three plants.
- One dedicated kiosk per plant (three total).
...
1. Employee enters employee number (or badge identifier in a later hardware
   integration).
2. Kiosk performs one-to-one facial verification. Automatic approval requires
   a provider that also reports a passed liveness check.
3. An evidence photo is captured for every attempt that becomes a punch.
4. After three failed facial attempts, the punch is accepted immediately and
   marked `identity_review`; there is no normal PIN fallback.
5. A camera/provider/connectivity failure also accepts the punch without
   consuming an employee attempt and opens review.
...
- Three kiosks can ingest the start-of-shift burst concurrently.
