...
import { parseProposalAccessCodes } from './accessCodes.js';
...
describe('proposal configuration', () => {
  it('registers unique valid slugs with the required default scope', () => {
    ...
    const proposal = getProposal('empacadora-demo');
  ...
  it('accepts only per-slug SHA-256 access hashes', () => {
    const hash = 'a'.repeat(64);
    expect(parseProposalAccessCodes(JSON.stringify({ 'empacadora-demo': hash }))).toEqual({ 'empacadora-demo': hash });
    expect(() => parseProposalAccessCodes('{bad')).toThrow(/JSON válido/);
    expect(() => parseProposalAccessCodes(JSON.stringify({ '../bad': hash }))).toThrow(/inválido/);
  });
});
